package com.mukul.jan.reactive.store.lib.store
...
import com.mukul.jan.reactive.store.lib.SubscriptionLifecycleBinding
...
fun <S : State, E : Event> Store<S, E>.consumeStateAsFlow(
    owner: LifecycleOwner? = null,
    scope: CoroutineScope,
    flow: suspend (Flow<S>) -> Unit
) {
    val store = this
    val channel = store.stateChannel(
        owner = owner,
    ...
    scope.launch {
        flow.invoke(channel.consumeAsFlow())
    }
}
...
fun <S : State, E : Event, T : Feature<S, E>> T.consumeStateAsFlow(
    owner: LifecycleOwner? = lifecycleOwner(),
    scope: CoroutineScope = coroutineScope(),
    flow: suspend (Flow<S>) -> Unit
): T {
    store().consumeStateAsFlow(
        owner = owner,
        scope = scope,
        flow = flow
    )
...
fun <S : State, E : Event> Fragment.consumeStateAsFlow(
    store: Store<S, E>,
    flow: suspend (Flow<S>) -> Unit
) {
    val fragment = this
...
fun <S : State, E : Event> ComponentActivity.consumeStateAsFlow(
    store: Store<S, E>,
    flow: suspend (Flow<S>) -> Unit
) {
    val activity = this
